...
import { List, ListItem, Modal, ConfirmationBanner, Navbar } from "./../components/";
import {
  editCompany,
  getCompanies,
  setCompanyId,
...
const HomeContainer = () => {
  ...
  const handleEdit = (desiredBudget) => {
    ...
    if (selectedCompany.budget < desiredNumber) {
      setIsValid(false);
      setValidationMessage("entered spent budget cannot be more than total budget");
      return;
    }
    if (selectedCompany.budget_spent > desiredNumber) {
        setIsValid(false);
        setValidationMessage("entered spent budget cannot be less than current spent budget");
        return;
    }
